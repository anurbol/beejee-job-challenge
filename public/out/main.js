...
import { localStorageItems, RequestBody } from "./contracts.js";
import { init, setState, state } from "./state.js";
...
export const render = (firstTime) => Promise.all(rootComponents.map(async (component) => {
    const fullname = `app-${component.name}`, el = firstTime
        ? document.getElementsByTagName(fullname)[0]
        : document.getElementById(fullname), tpl = document.createElement('template'), result = component(), HTML = result instanceof Promise ? (await result).trim() : result.trim();
    tpl.innerHTML = HTML;
...
const reducers = {
    async fetchUser() {
        ...
        if (key) {
            const response = await (await fetch(`./server/api.php?collection=sessions&key=${key}`)).json(), data = response.data;
            if (data) {
    ...
    async fetchTasks() {
        const orderByDirection = state.sortBy.direction === 'ascending' ? 'asc' : 'desc', page = state.pagination.currentPage, response = await (await fetch(`./server/api.php?collection=tasks&page=${page}&orderBy=${state.sortBy.column}&direction=${orderByDirection}`)).json(), data = response.data;
        if (data) {
            const tasks = data.data, paginationItems = constructPaginationItems(response);
    ...
    addTask: {
        ...
        async submit() {
            ...
            if (!state.addTask.validationFailed) {
                const response = await (await fetch('./server/api.php', {
                    method: 'POST',
    ...
    loginForm: {
        ...
        async submit() {
            ...
            if (state.login.validationFailed) {
                ...
                const response = await (await fetch('./server/api.php?collection=sessions', {
                    method: 'POST',
    ...
    editTask: {
        ...
        async startEdit(taskId) {
            ...
            const input = document.querySelector(`input[data-edit-task="${taskId}"]`);
            input.focus();
        },
        async save() {
            const task = state.tasks.find(t => t.id === state.editTask.taskId);
            const newTask = { ...task, title: state.editTask.value }, session_key = localStorage.getItem(localStorageItems.session_key);
            const response = await (await fetch('./server/api.php', {
                method: 'PATCH',
    ...
    async completeTask(taskId) {
        const task = state.tasks.find(t => t.id === taskId);
        const newTask = { ...task, done: !task.done }, session_key = localStorage.getItem(localStorageItems.session_key);
        const response = await (await fetch('./server/api.php', {
            method: 'PATCH',
            body: new RequestBody('tasks', {
                session_key,
